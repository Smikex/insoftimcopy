 ...
 */
if (!class_exists('QodeStartitStickyNavigationWalker')) {
	class QodeStartitStickyNavigationWalker extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			$wide_background = '';
			if($qode_startit_options['enable_wide_manu_background'] == "yes" ){
			...
			if($item->hide == ""){
				...
				if($item->featured_icon !== "" && $item->featured_icon == "icon_star"){
					$item_output .= '<span class=" qodef-featured-icon '. $item->featured_icon .'" aria-hidden="true"></span>';
				} else if ($item->featured_icon !== "" && $item->featured_icon == "New") {
                    $item_output .= '<span class=" qodef-featured-icon text " aria-hidden="true">' . esc_html__("New", "startit") . '</span>';
                }
